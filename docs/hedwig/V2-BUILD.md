...
- **Look** (replaced 2026-09-24): the spec is `docs/hedwig/design/DESIGN-AUDIT-2026-09-24.md` (§e colours and materials, §f radii/spacing/shadows), the build contract `docs/hedwig/design/REDESIGN-BUILD-2026-09-24.md`. Tokens live in `frontend/src/hedwig/theme/tokens.js` (`--hw-paper`, `--hw-content`, `--hw-glass`, `--hw-bar`, `--hw-field`, `--hw-hover`, `--hw-select`, `--hw-line`, `--hw-accent*`, `--hw-attention*`; the older names still resolve). Glass only for navigation (the rail) and toolbars (`bar`); lists, the reader and pages sit on opaque `content` sheets. Sheets radius 14, 8px window gutter and gaps, one cool light field on the ground. `tokens.test.js` checks every text token at ≥4.5:1 on the composited surfaces.
- **Type**: the system stack only (`-apple-system, BlinkMacSystemFont, "SF Pro Text", "Segoe UI Variable Text", "Segoe UI", InterVariable, Inter, Roboto, "Helvetica Neue", Arial, sans-serif`; the `system` font set is the Hedwig default, the Instrument set stays pickable and only it loads Google Fonts). No serif and no mono in the UI: titles are the body face at 600, reasons are 12px muted sans behind a glyph (`Reason`, never italic), times and counts use `font-variant-numeric: tabular-nums`; mono only for tracking numbers, one-time codes and code (`Code`). Sizes 11/12/13/14/17/20, weights 400/500/600. Avatars are allowed (`Avatar`: hashed hue, initials). Still no pills, no uppercase eyebrow labels (section headers are 11px/600 sentence case, `SectionLabel`), no left-border cards. One accent, blue, for selection, links, focus and the primary button; orange (`--hw-attention`) only as the attention signal: unread dot, Needs you, deadlines, flag. Icons are 16px Lucide-style line glyphs from `frontend/src/hedwig/icons.jsx`.
