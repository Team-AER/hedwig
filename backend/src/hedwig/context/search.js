...
export async function searchMessages(userId, opts = {}) {
  ...
  const hasFilter = Boolean(filters.entityId || filters.topicId || filters.after || filters.before);
  if (!q && !hasFilter) return { results: [] };
  const depth = Math.max(limit * 3, 30);
  const tsq = buildOrTsQuery(q);
  const [fts, vec, recent] = await Promise.all([
    fullTextIds(userId, tsq, filters, depth),
    q ? embedQuery(q).then((e) => (e ? vectorIds(userId, { vector: e.vector, dims: e.dims, model: e.model }, filters, depth) : [])) : [],
    // Within a person or topic, recency is itself a good signal ("what's the latest…").
    hasFilter || !q ? recentIds(userId, filters, depth) : [],
  ]);
  const merged = rrfMerge([{ ids: fts, weight: 1 }, { ids: vec, weight: 1 }, { ids: recent, weight: q ? 0.5 : 1 }]);
  const lite = await loadMessagesLite(userId, merged.slice(0, depth).map((m) => m.id));
  const score = new Map(merged.map((m) => [m.id, m.score]));
  return {
    results: lite.slice(0, limit).map((m) => ({ ...m, score: Math.round(score.get(m.id) * 1e5) / 1e5 })),
  };
}
